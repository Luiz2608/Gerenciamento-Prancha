...
export async function getPranchas() { 
  ...
  if (sb && isOnline()) { 
    ...
    const mergedRows = rows.map(r => {
      ...
      if (local) {
        return { ...r, plate: local.plate || r.plate, chassis: local.chassis || r.chassis, fleet: local.fleet || r.fleet, conjunto: local.conjunto || r.conjunto };
      }
      return r;
    });
