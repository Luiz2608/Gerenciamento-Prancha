...
export async function updateCaminhao(id, data) { await initLoad(); if (API_URL) { const r = await api(`/api/caminhoes/${id}`, { method: "PUT", body: JSON.stringify({ plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }) }); return await r.json(); } const { supabase: sb } = await import("./supabaseClient.js"); const payload = { plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }; if (sb && isOnline()) { const { data: row } = await sb.from("caminhoes").update(payload).eq("id", id).select().single(); const db = getDB(); const i = db.caminhoes.findIndex((d) => d.id === Number(id)); const localRow = { ...row, chassis: payload.chassis || row.chassis, plate: payload.plate || row.plate, fleet: payload.fleet || row.fleet }; if (i>=0) db.caminhoes[i] = localRow; else db.caminhoes.push(localRow); setDB(db); return localRow; } const db = getDB(); const i = db.caminhoes.findIndex((d) => d.id === Number(id)); if (i>=0) db.caminhoes[i] = { id: Number(id), ...payload }; else db.caminhoes.push({ id: Number(id), ...payload }); setDB(db); enqueue({ table: "caminhoes", op: "update", payload, localId: Number(id) }); return db.caminhoes[i>=0?i:db.caminhoes.length-1]; }
...
export async function getPranchas() { 
  ...
  if (sb && isOnline()) { 
    ...
    const mergedRows = rows.map(r => {
      ...
      if (local) {
        return { ...r, plate: local.plate || r.plate, chassis: local.chassis || r.chassis };
      }
      return r;
    });
...
export async function savePrancha(data) { await initLoad(); if (API_URL) { const r = await api("/api/pranchas", { method: "POST", body: JSON.stringify({ asset_number: data.asset_number || null, type: data.type || null, capacity: data.capacity != null ? Number(data.capacity) : null, year: data.year != null ? Number(data.year) : null, plate: data.plate || null, chassis: data.chassis || null, status: data.status || "Ativo" }) }); return await r.json(); } const { supabase: sb } = await import("./supabaseClient.js"); const payloadBase = { asset_number: data.asset_number || null, type: data.type || null, capacity: data.capacity != null ? Number(data.capacity) : null, year: data.year != null ? Number(data.year) : null, status: data.status || "Ativo" }; const payloadDb = { ...payloadBase, plate: data.plate || null, chassis: data.chassis || null }; if (sb && isOnline()) { let row = null; const ins1 = await sb.from("pranchas").insert([payloadDb]).select().single(); if (ins1.error || !ins1.data) { const ins2 = await sb.from("pranchas").insert([payloadBase]).select().single(); row = ins2.data ? { ...ins2.data, plate: payloadDb.plate, chassis: payloadDb.chassis } : null; } else { row = ins1.data; } const db = getDB(); if (row) { const idx = db.pranchas.findIndex((d) => d.id === row.id); if (idx>=0) db.pranchas[idx] = row; else db.pranchas.push(row); setDB(db); return row; } } const db = getDB(); const id = db.seq.pranchas++; const row = { id, ...payloadDb }; db.pranchas.push(row); setDB(db); enqueue({ table: "pranchas", op: "insert", payload: payloadDb, localId: id }); return row; }
export async function updatePrancha(id, data) { await initLoad(); if (API_URL) { const r = await api(`/api/pranchas/${id}`, { method: "PUT", body: JSON.stringify({ asset_number: data.asset_number || null, type: data.type || null, capacity: data.capacity != null ? Number(data.capacity) : null, year: data.year != null ? Number(data.year) : null, plate: data.plate || null, chassis: data.chassis || null, status: data.status || "Ativo" }) }); return await r.json(); } const { supabase: sb } = await import("./supabaseClient.js"); const payloadBase = { asset_number: data.asset_number || null, type: data.type || null, capacity: data.capacity != null ? Number(data.capacity) : null, year: data.year != null ? Number(data.year) : null, status: data.status || "Ativo" }; const payloadDb = { ...payloadBase, plate: data.plate || null, chassis: data.chassis || null }; if (sb && isOnline()) { let row = null; const up1 = await sb.from("pranchas").update(payloadDb).eq("id", Number(id)).select().single(); if (up1.error || !up1.data) { const up2 = await sb.from("pranchas").update(payloadBase).eq("id", Number(id)).select().single(); row = up2.data ? { ...up2.data, plate: payloadDb.plate, chassis: payloadDb.chassis } : null; } else { row = up1.data; } const db = getDB(); if (row) { const i = db.pranchas.findIndex((d) => d.id === Number(id)); if (i>=0) db.pranchas[i] = row; else db.pranchas.push(row); setDB(db); return row; } } const db = getDB(); const i = db.pranchas.findIndex((d) => d.id === Number(id)); if (i>=0) db.pranchas[i] = { id: Number(id), ...payloadDb }; else db.pranchas.push({ id: Number(id), ...payloadDb }); setDB(db); enqueue({ table: "pranchas", op: "update", payload: payloadDb, localId: Number(id) }); return db.pranchas[i>=0?i:db.pranchas.length-1]; }
