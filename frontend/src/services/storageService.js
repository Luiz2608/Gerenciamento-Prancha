...
export async function getViagens(opts = {}) {
  await initLoad();
  const db = getDB();
  const { startDate, endDate, driverId, destination, status, search, truckId, pranchaId, page = 1, pageSize = 10 } = opts;
  ...
  rows = rows.map((t) => ({ ...t, km_rodado: computeKm(t.km_start, t.km_end), horas: computeHours(t.date, t.start_time, t.end_time), total_cost: Number(t.fuel_liters || 0) * Number(t.fuel_price || 0) + Number(t.other_costs || 0) }));
...
export async function savePrancha(data) { await initLoad(); const db = getDB(); const id = db.seq.pranchas++; const row = { id, identifier: data.identifier || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, asset_number: data.asset_number || null, capacity: data.capacity != null ? Number(data.capacity) : null, notes: data.notes || null, status: data.status || "Ativo" }; db.pranchas.push(row); setDB(db); return row; }
export async function updatePrancha(id, data) { await initLoad(); const db = getDB(); const i = db.pranchas.findIndex((d) => d.id === Number(id)); if (i>=0) db.pranchas[i] = { id: Number(id), identifier: data.identifier || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, asset_number: data.asset_number || null, capacity: data.capacity != null ? Number(data.capacity) : null, notes: data.notes || null, status: data.status || "Ativo" }; setDB(db); return db.pranchas[i]; }
...
export async function exportCsv(filters = {}) {
  ...
  const rows = res.data;
  const header = ["id","data","motorista_id","caminhao_id","prancha_id","destino","tipo_servico","descricao","hora_saida","hora_retorno","km_inicial","km_final","status","km_rodado","horas_trabalhadas","combustivel_l","valor_combustivel","outros_custos","custo_total"];
  const lines = rows.map((t) => [t.id,t.date,t.driver_id,t.truck_id ?? "",t.prancha_id ?? "",t.destination || "",t.service_type || "",t.description || "",t.start_time || "",t.end_time || "",t.km_start ?? "",t.km_end ?? "",t.status,t.km_rodado,t.horas,t.fuel_liters ?? 0,t.fuel_price ?? 0,t.other_costs ?? 0,t.total_cost ?? 0]);
