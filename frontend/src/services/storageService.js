...
export async function getMotoristas() { await initLoad(); if (API_URL) { const r = await api("/api/motoristas"); const j = await r.json(); return Array.isArray(j) ? j : []; } return getDB().motoristas.slice().reverse(); }
export async function saveMotorista(data) { await initLoad(); if (API_URL) { const r = await api("/api/motoristas", { method: "POST", body: JSON.stringify({ name: data.name, cpf: data.cpf || null, cnh_category: data.cnh_category || null, status: data.status || "Ativo" }) }); return await r.json(); } const db = getDB(); const id = db.seq.motoristas++; const row = { id, name: data.name, cpf: data.cpf || null, cnh_category: data.cnh_category || null, status: data.status || "Ativo" }; db.motoristas.push(row); setDB(db); return row; }
export async function updateMotorista(id, data) { await initLoad(); if (API_URL) { const r = await api(`/api/motoristas/${id}`, { method: "PUT", body: JSON.stringify({ name: data.name, cpf: data.cpf || null, cnh_category: data.cnh_category || null, status: data.status || "Ativo" }) }); return await r.json(); } const db = getDB(); const i = db.motoristas.findIndex((d) => d.id === id); if (i>=0) db.motoristas[i] = { id, name: data.name, cpf: data.cpf || null, cnh_category: data.cnh_category || null, status: data.status || "Ativo" }; setDB(db); return db.motoristas[i]; }
export async function deleteMotorista(id) { await initLoad(); if (API_URL) { await api(`/api/motoristas/${id}`, { method: "DELETE" }); return { ok: true }; } const db = getDB(); db.motoristas = db.motoristas.filter((d) => d.id !== id); setDB(db); return { ok: true }; }
...
export async function getCaminhoes() { await initLoad(); if (API_URL) { const r = await api("/api/caminhoes"); const j = await r.json(); return Array.isArray(j) ? j : []; } return getDB().caminhoes.slice().reverse(); }
export async function saveCaminhao(data) { await initLoad(); if (API_URL) { const r = await api("/api/caminhoes", { method: "POST", body: JSON.stringify({ plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }) }); return await r.json(); } const db = getDB(); const id = db.seq.caminhoes++; const row = { id, plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }; db.caminhoes.push(row); setDB(db); return row; }
export async function updateCaminhao(id, data) { await initLoad(); if (API_URL) { const r = await api(`/api/caminhoes/${id}`, { method: "PUT", body: JSON.stringify({ plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }) }); return await r.json(); } const db = getDB(); const i = db.caminhoes.findIndex((d) => d.id === Number(id)); if (i>=0) db.caminhoes[i] = { id: Number(id), plate: data.plate || null, model: data.model || null, year: data.year != null ? Number(data.year) : null, chassis: data.chassis || null, km_current: data.km_current != null ? Number(data.km_current) : null, fleet: data.fleet || null, status: data.status || "Ativo" }; setDB(db); return db.caminhoes[i]; }
export async function deleteCaminhao(id) { await initLoad(); if (API_URL) { await api(`/api/caminhoes/${id}`, { method: "DELETE" }); return { ok: true }; } const db = getDB(); db.caminhoes = db.caminhoes.filter((d) => d.id !== Number(id)); setDB(db); return { ok: true }; }
...
export async function deletePrancha(id) { await initLoad(); if (API_URL) { await api(`/api/pranchas/${id}`, { method: "DELETE" }); return { ok: true }; } const db = getDB(); db.pranchas = db.pranchas.filter((d) => d.id !== Number(id)); setDB(db); return { ok: true }; }
