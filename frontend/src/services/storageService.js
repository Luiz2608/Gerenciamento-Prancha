...
export async function initLoad() {
  ...
  const [motoristas, viagens, destinos, tipos, usuarios, config] = await Promise.all([
...
export async function getViagens(opts = {}) {
  await initLoad();
  const db = getDB();
  const { startDate, endDate, driverId, destination, status, search, page = 1, pageSize = 10 } = opts;
...
export async function exportCsv(filters = {}) {
  ...
  const lines = rows.map((t) => [t.id,t.date,t.driver_id,t.destination || "",t.service_type || "",t.description || "",t.start_time || "",t.end_time || "",t.km_start ?? "",t.km_end ?? "",t.status,t.km_rodado,t.horas]);
